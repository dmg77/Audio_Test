...
class Uint8LogMelFeatureExtractor(object):
  ...
  def get_next_spectrogram(self, recorder, save_path=None):
    ...
    Args:
      ...
      save_path: (optional) path to save the spectrogram image to.
    ...
    self._spectrogram[:-self.frame_hop_spectra] = \
        self._spectrogram[self.frame_hop_spectra:]
    self._spectrogram[-self.frame_hop_spectra:] = self._get_next_spectra(
        recorder, self.frame_hop_spectra)
    if save_path is not None:
        plt.imsave(save_path, self._spectrogram)
    return (self._spectrogram[-self.frame_length_spectra:]
            .reshape((1, self.frame_length_spectra, self.num_mel_bins, 1))
            .astype(np.uint8))
...
def add_model_flags(parser):
  parser.add_argument(
      ...
      default="MEL_FNN_fixed_V6_quantized_edgetpu.tflite")
...
def classify_audio(audio_device_index, interpreter, labels_file,
  ...
  with recorder:
    last_detection = -1
    while not timed_out:
      spectrogram = feature_extractor.get_next_spectrogram(recorder, save_path='spectrogram.png')
